...
class InputValidator extends React.Component {
  ...
  validate() {
    const { children, fieldName, required, onValidated, customValidation } =
      this.props;
  ...
  onBlurValidate() {
    ...
    if (!_.isEmpty(onBlurValidators)) {
      onBlurValidators.forEach(
        (validatorCmp) => validatorCmp.validate && validatorCmp.validate()
      );
    }
  }
  ...
  render() {
    ...
    return (
      <div
        className={`validation ${this.props.className} ${this.state.inputState}`}
      >
        {children}
        {!isValid && (
